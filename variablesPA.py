...
def varCalculate(h,restX,uj,N,nUsers):
    ...
    for n in range(0,N):
        ...
        idx = np.where(restX[aux[0]:aux[comb-1]+1,3] != 0)[0];
...
def varCalculate_per_subcarrier(h,restX,uj,n,nUsers):
    comb = int((math.factorial(nUsers)/((math.factorial(nUsers-2))*math.factorial(2))));
    ...
    g1 = ((h1_norm**2)*rho/2);
    g2 = ((h2_norm**2)*rho/2);
    alpha = (np.abs(np.dot(np.conj(h_2).T,fc))**2)*(h2_norm**2);
    return u1,u2,g1,g2,alpha
